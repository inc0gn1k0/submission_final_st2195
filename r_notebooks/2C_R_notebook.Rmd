...
from attributes of the departure date, the scheduled departure and arrival times, the coordinates and distance between departure and planned arrival airports, and the carrier. 
...
#Diverted percentage based feature preparation and Carrier feature preparation....
...
df_ranked_divertedcount_2004_2007 <- do.call(rbind, temp_storage_ranked_1) %>%
  group_by(Airport) %>%
  summarise(DivertedCount = sum(DivertedCount)) %>%
  arrange(desc(DivertedCount)) %>%
  ungroup()

sum_diverted_count <- sum(df_ranked_divertedcount_2004_2007$DivertedCount)
df_ranked_divertedcount_2004_2007$DivertedCount <- df_ranked_divertedcount_2004_2007$DivertedCount/sum_diverted_count
percent_check <- sum(df_ranked_divertedcount_2004_2007$DivertedCount)

df_ranked_divertedcount_2004_2007 <- df_ranked_divertedcount_2004_2007 %>% rename(DivertedCount_pct = DivertedCount)

# Optional: Add rank if needed. Note: Commented out as mentioned not improving the model.
...
print(head(df_ranked_divertedcount_2004_2007, 10))
print(tail(df_ranked_divertedcount_2004_2007, 13))
...
df_ranked_carrier_diverted_2004_2007 <- do.call(rbind, temp_storage_ranked_2) %>%
...
sum_carrier_diverted_count <- sum(df_ranked_carrier_diverted_2004_2007$DivertedCount)
...
percent_check <- sum(df_ranked_carrier_diverted_2004_2007$DivertedCount)

df_ranked_carrier_diverted_2004_2007 <- df_ranked_carrier_diverted_2004_2007 %>% rename(Carrier_pct_Diverted = DivertedCount)


print(percent_check)
# Display top 10 and bottom 10 airports
print(head(df_ranked_carrier_diverted_2004_2007, 10))
print(tail(df_ranked_carrier_diverted_2004_2007, 13))
...
df_ranked_depcount_2004_2007 <- do.call(rbind, temp_storage_ranked_1) %>%
  group_by(Airport) %>%
  summarise(DepDelayCount = sum(DepDelayCount)) %>%
  arrange(desc(DepDelayCount))

sum_depcount <- sum(df_ranked_depcount_2004_2007$DepDelayCount)

df_ranked_depcount_2004_2007$DepDelayCount <- df_ranked_depcount_2004_2007$DepDelayCount/sum_depcount

df_ranked_depcount_2004_2007 <- rename(df_ranked_depcount_2004_2007, DepDelayCount_pct= DepDelayCount)
percent_check <- sum(df_ranked_depcount_2004_2007$DepDelayCount)

print(percent_check)
print(head(df_ranked_depcount_2004_2007, 10))
print(tail(df_ranked_depcount_2004_2007, 10))
...
df_ranked_arrcount_2004_2007 <- do.call(rbind, temp_storage_ranked_3) %>%
  group_by(Airport) %>%
  summarise(ArrDelayCount = sum(ArrDelayCount)) %>%
  arrange(desc(ArrDelayCount))

sum_arrcount <- sum(df_ranked_arrcount_2004_2007$ArrDelayCount)

df_ranked_arrcount_2004_2007$ArrDelayCount <- df_ranked_arrcount_2004_2007$ArrDelayCount/sum_arrcount

df_ranked_arrcount_2004_2007 <- rename(df_ranked_arrcount_2004_2007,ArrDelayCount_pct= ArrDelayCount)
percent_check <- sum(df_ranked_arrcount_2004_2007$ArrDelayCount)

print(percent_check)
print(head(df_ranked_arrcount_2004_2007, 10))
print(tail(df_ranked_arrcount_2004_2007, 10))
...
# Calculate the total number of rows in your final data frame
...
```{r Q2C MODELv1 - ROC-AUC and coefficients...which we have to use for plotting the bar plot - Part 6.1}
...
# 
# # Load the dataset
# df <- fread('/Users/nikolaijaniszewsky/Desktop/My_Stuff/UOL_Course_Content/Academia/2024/ST2195/ST2195_coursework/raw_data/complete-airplane-feature-set-R.csv')
# 
# # Select required columns and ensure it's a data.table
# selected_df <- df[, .(Year, Month, DayOfWeek, CRSArrTime, CRSDepTime, Distance, DivertedCount_pct_Dest, DepDelayCount_pct_Dest, ArrDelayCount_pct_Dest, Carrier_pct_Diverted_UniqueCarrier, Diverted)]
# 
...
#   theme_minimal()
# 
# ggsave("/Users/nikolaijaniszewsky/Desktop/My_Stuff/UOL_Course_Content/Academia/2024/ST2195/ST2195_coursework/submission/R-2C-ROC-2004-2007-EXP.png", plot = roc_curve, width = 10, height = 8)
...
```{r Q2C MODELv3 - Full Legend - Part 6.2}
# Clear existing workspace
rm(list = ls())
...
# Note: You might need to adjust the 'vjust' and 'nudge_y' for your specific plot
...
```{r Q2C Final Coefficient Plot...Plotting barplot - Part 6.3}
...
# Assuming results_list is your list of models for different years
...
```{r - Part 8 - Improving The Model by tuning hyperparameters}
library(glmnet)
